...
func TestDeepSeekModel_ValidConfig(t *testing.T) {
	config := DeepSeekModelConfig{
		APIKey: "test-key",
	}
	...
	model, err := NewDeepSeekModel(config)
	if err != nil {
		t.Fatalf("Failed to create DeepSeek model with valid config: %v", err)
	}
...
func TestDeepSeekModel_GenerateEmbeddings_NotSupported(t *testing.T) {
	config := DeepSeekModelConfig{
		APIKey: "test-key",
	}
	...
	if err.Error() != "embeddings are not supported by DeepSeek models" {
		t.Errorf("Expected 'embeddings are not supported by DeepSeek models', got '%s'", err.Error())
	}
}

func TestDeepSeekModel_GenerateImage_NotSupported(t *testing.T) {
	config := DeepSeekModelConfig{
		APIKey: "test-key",
	}
	...
	if err.Error() != "image generation is not supported by DeepSeek models" {
		t.Errorf("Expected 'image generation is not supported by DeepSeek models', got '%s'", err.Error())
	}
}
